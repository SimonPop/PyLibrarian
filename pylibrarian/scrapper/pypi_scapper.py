...
class PypiScrapper():
    def __init__(self, output_file: str = None, package_source: str = "packages.txt", project: str = None):

        # Package source:
        self.package_source = Path(__file__).parent.parent.parent / "data" / package_source
    ...
    def write_row(self, package: str, requirements: List[str]):
        ...
        if len(rows) > 0:
            errors = self.client.insert_rows_json(f"{self.project}.pylibrarian.requirements", rows)
